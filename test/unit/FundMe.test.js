...
describe("FundMe", function () {
  ...
  describe("withdraw", function () {
    ...
    it("Should allow the owner to withdraw a specified amount", async () => {
      const initialOwnerBalance = await fundMe.provider.getBalance(deployer);
      ...
      assert.equal(finalOwnerBalance.toString(), expectedOwnerBalance.toString());
    });
    ...
    it("Should revert if someone other than the owner tries to withdraw", async function () {
      ...
      await expect(fundMeConnectedContract.withdraw(sendValue)).to.be.revertedWith("You're not the owner!");
    });
  ...
  describe("getters", function () {
    ...
    it("Should return the correct contract balance", async () => {
      ...
      assert.equal(contractBalance.toString(), sendValue.toString(), "Contract balance should match");
    });
